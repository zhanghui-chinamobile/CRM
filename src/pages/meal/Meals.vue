                ...
                this.$http.getSimOfferUseDetail(params).then((res) => {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (type == 1) {
                            ...
                            this.count.llDetail = data[i].usedValue + "M/" +  data[i].totalValue + "M";

                        } else if (type == 2) {
                            this.count.dxCount = parseInt((data[i].usedValue / data[i].totalValue) * 100);
                            this.count.dxDetail = data[i].usedValue + "条/" +  data[i].totalValue + "条";
                        }
                        if (type == 3) {
                            this.count.yyCount = parseInt((data[i].usedValue / data[i].totalValue) * 100);
                            this.count.yyDetail = data[i].usedValue + "分钟/" +  data[i].totalValue + "分钟";
                        }
                    }
                }).catch(error => {
                });
