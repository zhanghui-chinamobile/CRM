        ...
        <section>
            <el-col :span="24" class="toolbar">
                <el-form :inline="true" :model="formInline" class="demo-form-inline" size="small">
                    ...
                    <el-form-item class="toolbar-button">
                      ...
                      <el-button type="primary" @click="purchaseMoreService" icon="el-icon-upload" size="small">批量订购</el-button>
                    </el-form-item>
                </el-form>
            </el-col>
            <c-table :table="tableData"  @onHandleSelectionChange="getCopyItemRow" ref="itemTable" style="padding-top: 10px">
                <template slot-scope="props" slot="button-operation" style="width: 300px">
                    ...
                    <el-tooltip effect="dark" content="查看套餐列表" placement="top-start">
                        <el-button type="success"  style="width: 60px !important" @click="getInfoList(props.obj.row.id)" v-if="props.obj.row.status == '已销户'" disabled>套餐管理</el-button>
                        <el-button type="success"  style="width: 60px !important" @click="getInfoList(props.obj.row.id)">套餐管理</el-button>
                    </el-tooltip>
                </template>
            </c-table>
            <el-col :span="24" class="toolbar">
          ...
          <el-dialog title="车企选择" :visible.sync="userDialogVisible" width="80%" :close-on-press-escape="false" append-to-body>
            ...
            <el-dialog :title="title" :visible.sync="infoListVisible" width="80%" :close-on-click-modal="false">
              <sim-menu-list ref="SimMenuList" :enterpriseId="enterpriseId" :simId="id" v-if="infoListVisible"></sim-menu-list>
            </el-dialog>
            <el-dialog :title="title" :visible.sync="parchServiceVisible" width="80%" :close-on-click-modal="false">
              <billing ref="Billing" :enterpriseId="enterpriseId" :userIds="simIds" :consumerType="2" v-if="parchServiceVisible" @closeBilling = "parchServiceVisible = false"></billing>
            </el-dialog>
            ...
            <el-dialog :title="title" :visible.sync="purchaseMoreServiceVisible" width="50%">
              <i class="el-icon-download" style="float: right;margin-right: 10px"><a href="/static/excel/model.txt" download="download">下载模板文件</a></i>
              <form id="uploadForm">
                <input type="file" id="fileUpload">
              </form>
              <div slot="footer" class="dialog-footer" style="text-align: center;margin: auto">
                <el-button type="primary"  @click="purchaseMore">确定</el-button>
              </div>
            </el-dialog>
        </section>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                addSimVisible: false,
                purchaseMoreServiceVisible:false,
                enterpriseId: 0,
                enterpriseName:'',
                operationTitle: "",
                info: {},
        ...
        methods: {
            ...
            getCopyItemRow(val) {
                ...
                for (var i = 0; i < val.length; i++) {
                    if (val[i].status != "测试期"){
                        this.$message({
                            message: "SIM卡【" + val[i].simid + "】不在测试期" +  "，无法进行套餐订购",
                            type: "error",
                        });
            ...
            purchaseMoreService(){
                this.purchaseMoreServiceVisible = true;
            },
            queryUserType(){
                let params={}
            ...
            purchaseMore() {
              ...
              reader.onload = function(oFREvent){
                ...
                that.$http.purchaseMoreService(params).then((res) => {
                  document.getElementById('uploadForm')&&document.getElementById('uploadForm').reset();
                  that.purchaseMoreServiceVisible = false;
